...
public class PayloadHelper {
    ...
    public static Stream<IndexedEntry> getIndexedJsonObjectStream(MessageContext mc, String jsonPath)
            throws JaxenException {

        JsonElement evaluationResultJson = getPayloadJsonElement(mc, jsonPath);
        return getJsonObjectStreamWithIndex(
                parser.parse(evaluationResultJson.getAsJsonArray().get(0).getAsString()).getAsJsonObject());
    }

    public static Stream<Map.Entry<String, JsonElement>> getJsonObjectStream(MessageContext mc, String jsonPath)
            throws JaxenException {

        JsonElement evaluationResultJson = getPayloadJsonElement(mc, jsonPath);
        return getJsonObjectStream(
                parser.parse(evaluationResultJson.getAsJsonArray().get(0).getAsString()).getAsJsonObject());
    }
